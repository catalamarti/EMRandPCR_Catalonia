...
plot(time,cas7_Pc(:,end),'linewidth',2,'Color',col(1,:))
hold on
plot(time,prop(end)*cas7_CC(:,end),'linewidth',2,'Color',col(2,:))
ax.YLim(1) = 0;
YL = ax.YLim;

opt = {'Pc','CC'};
for s = 1:length(opt)
    eval(['x = [cas7_' opt{s} '(:,end)+icas7_' opt{s} '(:,end); flip(cas7_' opt{s} '(:,end)-icas7_' opt{s} '(:,end))];'])
    x = x';
    if s==2
        x = x*prop(end);
    end
...
plot(time,rho_Pc(:,end),'linewidth',2,'Color',col(1,:))
hold on
plot(time,rho_CC(:,end),'linewidth',2,'Color',col(2,:))

opt = {'Pc','CC'};
for s = 1:length(opt)
    eval(['x = [rho_' opt{s} '(:,end)+irho_' opt{s} '(:,end); flip(rho_' opt{s} '(:,end)-irho_' opt{s} '(:,end))];'])
    x = x';
    x(x<0) = 0;
    x(x>5) = 5;
    fill([time flip(time)],x,'r','FaceColor',0.5+0.5.*col(s,:),'EdgeColor','none','FaceAlpha',0.5)
...
plot(time,A14_Pc(:,end),'linewidth',2,'Color',col(1,:))
hold on
plot(time,prop(end)*A14_CC(:,end),'linewidth',2,'Color',col(2,:))
ax.YLim(1) = 0;
YL = ax.YLim;

opt = {'Pc','CC'};
for s = 1:length(opt)
    eval(['x = [A14_' opt{s} '(:,end)+iA14_' opt{s} '(:,end); flip(A14_' opt{s} '(:,end)-iA14_' opt{s} '(:,end))];'])
    x = x';
    if s==2
        x = x*prop(end);
    end
...
plot(time,EPG_Pc(:,end),'linewidth',2,'Color',col(1,:))
hold on
plot(time,prop(end)*EPG_CC(:,end),'linewidth',2,'Color',col(2,:))
ax.YLim(1) = 0;
YL = ax.YLim;

opt = {'Pc','CC'};
for s = 1:length(opt)
    eval(['x = [EPG_' opt{s} '(:,end)+iEPG_' opt{s} '(:,end); flip(EPG_' opt{s} '(:,end)-iEPG_' opt{s} '(:,end))];'])
    x = x';
    if s==2
        x = x*prop(end);
    end
...
print(f2a,'./Figures&Tables/Figure_indictors_CAT_F1.png','-dpng','-r600')
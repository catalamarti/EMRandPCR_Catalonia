...
dy = iEPG_Pc(:,id);
xx = [xx flip(xx)];
yy = [yy-dy; flip(yy+dy)]; yy(yy>10000) = 10000; yy(yy<0) = 0;
fill(xx,yy,'g','FaceColor',0.5*[1 1 1],'EdgeColor','none','FaceAlpha',0.5)
xx = time;
yy = EPG_CC(:,id);
dy = iEPG_CC(:,id);
xx = [xx flip(xx)];
yy = [yy-dy; flip(yy+dy)]; yy(yy>10000) = 10000; yy(yy<0) = 0;
fill(xx,yy,'g','FaceColor',0.5+0.5*[0.6 0.1 0],'EdgeColor','none','FaceAlpha',0.5)
...
dyp = nm*imorts7(:,id,2);
xx = [xx flip(xx)];
yy = [yy-dym; flip(yy+dyp)]; yy(yy>10000) = 10000; yy(yy<0) = 0;
fill(xx,yy,'g','FaceColor',0.5+0.5*[0 0 1],'EdgeColor','none','FaceAlpha',0.5)
xx = time;
yy = Hosp7(:,id);
dy = iHosp7(:,id);
xx = [xx flip(xx)];
yy = [yy-dy; flip(yy+dy)]; yy(yy>10000) = 10000; yy(yy<0) = 0;
fill(xx,yy,'g','FaceColor',0.5+0.5*[0 1 0],'EdgeColor','none','FaceAlpha',0.5)
xx = time;
yy = nu*UCIs7(:,id);
dy = nu*iUCIs7(:,id);
xx = [xx flip(xx)];
yy = [yy-dy; flip(yy+dy)]; yy(yy>10000) = 10000; yy(yy<0) = 0;
fill(xx,yy,'g','FaceColor',0.5+0.5*[1 0 0],'EdgeColor','none','FaceAlpha',0.5)
...
print(f,'./Figures&Tables/Figure_EPG_HID_CAT_F4.png','-dpng','-r600')